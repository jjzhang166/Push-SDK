...
static FFDrawContext drawColorCtx;
static FFDrawColor fontColor;          		///< foreground color
static FT_Library ftLibrary = NULL;         ///< freetype font library handle
static FT_Face ftFace = NULL;               ///< freetype font face handle
//static FT_Stroker ftStroker;             	///< freetype stroker handle
static struct AVTreeNode* glyphsArr = NULL; ///< rendered glyphs, stored using the UTF-32 char code
static int max_glyph_w = 0;                	///< max glyph width
static int max_glyph_h = 0;                	///< max glyph height
static int useKerning = 0;	
static int tabSize = 0;

static uint8_t textAlpha = 255;				///< text alpha
 ...
 */
static int load_glyph(Glyph **glyph_ptr, uint32_t code)
{
    ...
    if (FT_Load_Char(ftFace, code, FT_LOAD_DEFAULT))
        return AVERROR(EINVAL);
    ...
    if (FT_Get_Glyph(ftFace->glyph, &glyph->glyph)) {
        ret = AVERROR(EINVAL);
        goto error;
    }
    ...
    glyph->advance     = ftFace->glyph->advance.x >> 6;
    ...
    av_tree_insert(&glyphsArr, glyph, glyph_cmp, &node);
...
static int load_font_file(const char *path, int index)
{
    int err = FT_New_Face(ftLibrary, path, index, &ftFace);
    if (err) {
		ftFace = NULL;
        av_log(NULL, AV_LOG_ERROR, "Could not load font \"%s\": %s\n",
               path, FT_ERRMSG(err));
        return AVERROR(EINVAL);
    }
    return 0;
 ...
 int avfilter_set_font(const char* fontfile, const char* color, enum AVPixelFormat format, int size, float alpha)
 {
	int ret = 0;
	int reloadFont = 0;
	Glyph *glyph = NULL;
	...
	// Parse font color
	ret = av_parse_color(fontColor.rgba, color, strlen(color), NULL);
    if (ret) {
		av_log(NULL, AV_LOG_ERROR, "Parse color string failed.\n");
		return ret;
	}
	
	ff_draw_init(&drawColorCtx, format, 0);
    ff_draw_color(&drawColorCtx, &fontColor, fontColor.rgba);
	
	// Init free type library
	if(!ftLibrary) {
		if ((ret = FT_Init_FreeType(&ftLibrary))) {
			av_log(NULL, AV_LOG_ERROR,
				   "Could not initialize FreeType library, error: %s\n", FT_ERRMSG(ret));
			return AVERROR(EINVAL);
		}
	}
	
	if(!fontFileName) {
		fontFileName = av_strdup(fontfile);
		reloadFont = 1;
	} else if(av_strcasecmp(fontFileName, fontfile) != 0) {
		av_free(fontFileName);
		fontFileName = av_strdup(fontfile);
		reloadFont = 1;
		FT_Done_Face(ftFace);
	}
	...
	// load font file
	if(reloadFont) {
		ret = load_font_file(fontfile, 0);
		useKerning = FT_HAS_KERNING(ftFace);
	}
	
	// set font size
	if(fontSize != size || reloadFont) {
		tabSize = 4;
		if(size == 0) {
			fontSize = 14;
		} else {
		...
		if ((ret = FT_Set_Pixel_Sizes(ftFace, 0, fontSize))) {
			av_log(NULL, AV_LOG_ERROR, "Could not set font size to %d pixels: %s\n",
				   fontSize, FT_ERRMSG(ret));
			return AVERROR(EINVAL);
		}
		
		av_tree_enumerate(glyphsArr, NULL, NULL, glyph_enu_free);
		av_tree_destroy(glyphsArr);
		glyphsArr = NULL;
		max_glyph_w = 0;
		max_glyph_h = 0;
		...
		if ((ret = load_glyph(&glyph, ' ')) < 0) {
			av_log(NULL, AV_LOG_ERROR, "Could not set tabsize.\n");
			return ret;
		}
 ...
 // variables used in draw_text call (should be update before draw each string)
 static int nb_chRelativePos = 0;
 static FT_Vector *chRelativePos = NULL;
 
 static int draw_glyphs(AVFrame *frame,
					   const char *text, FT_Vector* pos,
                       ...
                       int x, int y)
{
    ...
    for (i = 0, p = text; *p; i++) {
        ...
        dummy.code = code;
        glyph = av_tree_find(glyphsArr, &dummy, (void *)glyph_cmp, NULL);
        ...
        x1 = chRelativePos[i].x+pos->x+x;
        y1 = chRelativePos[i].y+pos->y+y;

        ff_blend_mask(&drawColorCtx, color,
...
static void update_color_with_alpha(FFDrawColor *color, const FFDrawColor incolor)
{
    *color = incolor;
    color->rgba[3] = (color->rgba[3] * textAlpha) / 255;
    ff_draw_color(&drawColorCtx, color, color->rgba);
}

 int avfilter_draw_text(AVFrame *frame, const char *text, int posx, int posy, int align, int text_rect_w, int text_rect_h)
 {
	uint32_t code = 0, prev_code = 0;
    int x = 0, y = 0, i = 0, ret;
    int max_text_line_w = 0, len;
    int box_w, box_h;	// text box
	...
	if ((len = strlen(text)) > nb_chRelativePos) {
        if (!(chRelativePos =
              av_realloc(chRelativePos, len*sizeof(*chRelativePos))))
            return AVERROR(ENOMEM);
        nb_chRelativePos = len;
    }
    ...
    for (i = 0, p = text; *p; i++) {
        ...
        /* get glyph */
        dummy.code = code;
        glyph = av_tree_find(glyphsArr, &dummy, glyph_cmp, NULL);
        if (!glyph) {
            load_glyph(&glyph, code);
        }
    ...
    max_glyph_h = y_max - y_min;
    max_glyph_w = x_max - x_min;
    ...
    for (i = 0, p = text; *p; i++) {
        ...
        /* kerning */
        if (useKerning && prev_glyph && glyph->code) {
            FT_Get_Kerning(ftFace, prev_glyph->code, glyph->code,
                           ft_kerning_default, &delta);
            x += delta.x >> 6;
        }

        /* save position */
        chRelativePos[i].x = x + glyph->bitmap_left;
        chRelativePos[i].y = y - glyph->bitmap_top + y_max;
        if (code == '\t') x  = (x / tabSize + 1)*tabSize;
        else              x += glyph->advance;
    }

    max_text_line_w = FFMAX(x, max_text_line_w);

    update_color_with_alpha(&fontcolor, fontColor);
   
    box_w = FFMIN(width - 1 , max_text_line_w);
    box_h = FFMIN(height - 1, y + max_glyph_h);

    FT_Vector pos;
    pos.x = posx;
    pos.y = posy;
    if(text_rect_w > 0 && text_rect_h > 0)  {
	    if(align ==0 ) {			// left
	    } else if(align ==1 ) {	// center
	    	pos.x += (text_rect_w-box_w)/2;
	    } else {				// right
	    	pos.x += (text_rect_w-box_w);
	    }
	    pos.y +=  (text_rect_h-box_h)/2;
	    //av_log(NULL, AV_LOG_INFO, "Original pos(%d, %d), Rect(%dx%d), BBox(%dx%d), TextPos(%d, %d).\n", posx, posy, text_rect_w, text_rect_h, box_w, box_h, pos.x, pos.y);
    }
	
    if ((ret = draw_glyphs(frame, text, &pos, width, height,
                           &fontcolor, 0, 0)) < 0) {
		return ret;					   
	}
 ...
 int avfilter_uninit_font(void)
 {
	av_freep(&chRelativePos);
	nb_chRelativePos = 0;
