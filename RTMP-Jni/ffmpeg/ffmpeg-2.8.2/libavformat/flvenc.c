...
/**
 * Absolute timestamp related functions
 */
 static int enable_abs_time = 1;
void av_enable_rtmp_abs_time(int b_enable)
{
	enable_abs_time = b_enable;
}

int av_rtmp_abs_time_enabled(void)
{
      return enable_abs_time;
}
...
{
	int64_t curMs = av_gettime()/1000;
	struct tm *localTm;
	time_t curSecs = curMs/1000;
	localTm = localtime(&curSecs);
       int64_t todayMs = localTm->tm_hour*3600 + localTm->tm_min*60 + localTm->tm_sec;
	todayMs = todayMs*1000 + curMs%1000;
	return todayMs;
}

static int write_absolute_time_notify(AVFormatContext *s, uint32_t ts, int64_t abs_time)
{
	AVIOContext *pb      = s->pb;
	   
	char timeBuf[12] = {0};
	int bufLen = 14;
	int totalSecs = abs_time/1000;
	int msecs = abs_time%1000;
	int secs = totalSecs%60;
	int hours = totalSecs/3600;
	int mins = (totalSecs%3600)/60;
	
	av_strlcatf(timeBuf, 12, "tm%02d%02d%02d%03d", hours, mins, secs, msecs);
	...
	avio_wb32(pb, bufLen + 11);
	
	av_log(s, AV_LOG_INFO, "****** Write absolute time: %s, ms:%I64d\n", timeBuf, abs_time);
	return bufLen;	// data length
	 
}

static int video_packets_num = 0;
...
static int flv_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    switch (enc->codec_type) {
    case AVMEDIA_TYPE_VIDEO:
		if(av_rtmp_abs_time_enabled()) {
			if(pkt->abs_time >= 0) {
				write_absolute_time_notify(s, ts, pkt->abs_time);
			} /*else {
				if(video_packets_num%26 == 0) {
					video_packets_num = 1;
					int64_t curMs = get_current_ms();
					write_absolute_time_notify(s, ts, curMs);
				}
				video_packets_num++;
			}*/
		}
