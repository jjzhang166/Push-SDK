...
////printf("Eoollo %s:Line %d scale %d duration %d\n", __func__, __LINE__, track->timescale, track->track_duration);
...
static int mov_write_dvc1_structs(MOVTrack *track, uint8_t *buf)
{
    ...
    /* framerate */
////printf("Eoollo %s:Line %d avg frame rate num %d den %d\n", __func__, __LINE__, track->st->avg_frame_rate.num, track->st->avg_frame_rate.den);
...
{
////printf("Eoollo %s:Line %d codec time den %d num %d\n", __func__, __LINE__, st->codec->time_base.den, st->codec->time_base.num);
...
//printf("Eoollo %s:Line %d nb_frames %d time_base den %d num %d frame_duration %d nb_frames %d\n", __func__, __LINE__, nb_frames, track->enc->time_base.den, track->enc->time_base.num, frame_duration, nb_frames);
    if (nb_frames > 255) {
...
//AVStream *st = track->st;
//if(NULL != st)
//printf("Eoollo %s:Line %d Track AVStream time_base den %d num %d\n", __func__, __LINE__, st->time_base.den, st->time_base.num);
//if(NULL != st && NULL != st->codec)
//printf("Eoollo %s:Line %d Track AVStream codec time_base den %d num %d codec bit_rate %d\n", __func__, __LINE__, st->codec->time_base.den, st->codec->time_base.num, st->codec->bit_rate);

//printf("Eoollo %s:Line %d mode %d entry %d timescale %d time %lld track_duration %lld end %d count %ld size %ld cCount %ld keyfarmes %d flags %d timecode_flags %d track_id %d tag %d mono %d\n", __func__, __LINE__, track->mode, track->entry, track->timescale, track->time, track->track_duration, track->last_sample_is_subtitle_end, track->sample_count, track->sample_size, track->chunkCount, track->has_keyframes, track->flags, track->timecode_flags, track->track_id, track->tag, track->multichannel_as_mono);

//printf("Eoollo %s:Line %d vos_len %d capacity %d audio_vbr %d height %d tag %d id %d start dts %lld cts %lld end pts %lld, hint_track %d src_track %d rtp_ts %d ts_unwrapped %lld max_packet_size %d default duration %lld sample_flags %d size %d\n", __func__, __LINE__, track->vos_len, track->cluster_capacity, track->audio_vbr,track->height, track->tref_tag, track->tref_id, track->start_dts, track->start_cts, track->end_pts, track->hint_track, track->src_track, track->prev_rtp_ts, track->cur_rtp_ts_unwrapped, track->max_packet_size, track->default_duration, track->default_sample_flags, track->default_size);

//printf("Eoollo %s:Line %d data_offset %lld frag_start %lld discont %d flushed %d info %d capacity %d \n", __func__, __LINE__, track->data_offset, track->frag_start, track->frag_discont, track->entries_flushed, track->nb_frag_info, track->frag_info_capacity);
/////////////////////////////////////////////////
...
//printf("Eoollo %s:Line %d scale %d track_duration %d duration %lld\n", __func__, __LINE__, track->timescale, track->track_duration, duration);
...
//printf("Eoollo %s:Line %d scale %d duration %d delay %lld\n", __func__, __LINE__, track->timescale, track->track_duration, delay);
...
static int mov_write_mvhd_tag(AVIOContext *pb, MOVMuxContext *mov)
{
    ...
    for (i = 0; i < mov->nb_streams; i++) {
//printf("Eoollo %s:Line %d scale %d duration %d\n", __func__, __LINE__, mov->tracks[i].timescale, mov->tracks[i].track_duration);
...
static int mov_write_chpl_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        AVDictionaryEntry *t;
//printf("Eoollo %s:Line %d c time_base den %d num %d\n", __func__, __LINE__, c->time_base.den, c->time_base.num);
...
//printf("Eoollo %s:Line %d frame_rate %d bitrate audio %d video %d\n", __func__, __LINE__, frame_rate, audio_kbitrate, video_kbitrate);
...
static int mov_flush_fragment(AVFormatContext *s)
{
    ...
    if (!(mov->flags & FF_MOV_FLAG_FRAGMENT))
        return 0;
//printf("Eoollo %s:Line %d scale %d duration %d\n", __func__, __LINE__, mov->tracks[0].timescale, mov->tracks[0].track_duration);
...
//printf("Eoollo %s:Line %d track_duration %d dts %lld start_dts %lld pkt->duration %d\n", __func__, __LINE__, trk->track_duration, pkt->dts, trk->start_dts, pkt->duration);
...
//printf("Eoollo %s:Line %d enc time_base den %d num %d\n", __func__, __LINE__, track->enc->time_base.den, track->enc->time_base.num);
...
//printf("Eoollo %s:Line %d enc ID %d time_base den %d num %d bitrate %d\n", __func__, __LINE__, track->enc->codec_id, track->enc->time_base.den, track->enc->time_base.num, track->enc->bit_rate);
...
//printf("Eoollo %s:Line %d Video Stream : CodecID %d bitrate %d W %d H %d st->time_base num :%d den :%d codec->time_base num:%d den:%d\n", __func__, __LINE__, st->codec->codec_id, st->codec->bit_rate, st->codec->width,  st->codec->height, st->time_base.num, st->time_base.den, st->codec->time_base.num, st->codec->time_base.den);
//printf("Eoollo %s:Line %d video_track_timescale %d\n", __func__, __LINE__, mov->video_track_timescale);
...
//printf("Eoollo %s:Line %d track->timescale %d\n", __func__, __LINE__, track->timescale);
...
static int mov_write_trailer(AVFormatContext *s)
{
    ...
    int64_t moov_pos;
//////////
		//MOVTrack *etrk = &mov->tracks[0];
		//printf("Eoollo %s:Line %d mode %d entry %d timescale %d time %lld track_duration %lld count %ld size %ld chunkCount %d\n", __func__, __LINE__, etrk->mode, etrk->entry, etrk->timescale, etrk->time, etrk->track_duration, etrk->sample_count, etrk->sample_size, etrk->chunkCount);
//////////////
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        MOVTrack *trk = &mov->tracks[i];
//printf("Eoollo %s:Line %d avg frame rate num %d den %d\n", __func__, __LINE__, trk->st->avg_frame_rate.num, trk->st->avg_frame_rate.den);
        if (trk->enc->codec_id == AV_CODEC_ID_MOV_TEXT &&
