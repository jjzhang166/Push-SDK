    ...
    for (;; avio_skip(s->pb, 4)) {
        ...
        if (type == FLV_TAG_TYPE_AUDIO) {
            ...
            if (size > 13 + 1 + 4) { // Header-type metadata stuff
                ...
                AMFDataType type = avio_r8(s->pb);
				uint8_t commandbuffer[64];
                if(type == AMF_DATA_TYPE_STRING) {
                    if(amf_get_string(s->pb,commandbuffer, sizeof(commandbuffer)) > 0) {
                        av_log(s, AV_LOG_INFO, "Read flv packet, absTime:%s.\n",commandbuffer);
						char chhour[3]={0}, chmin[3]={0}, chsec[3]={0}, chmsec[4]={0};
						int ihour = 0, imin = 0, isec = 0, imsec = 0;    
						strncpy(chhour, commandbuffer + 2, 2);    
						strncpy(chmin, commandbuffer + 4, 2);    
						strncpy(chsec, commandbuffer + 6, 2);    
						strncpy(chmsec, commandbuffer + 8, 3);    
						ihour = atoi(chhour);	
						imin = atoi(chmin);    
						isec = atoi(chsec);    
						imsec = atoi(chmsec); 
						flv->last_abs_time= (ihour * 3600 + imin * 60 + isec) * 1000 + imsec;
                    }
...
retry_duration:
	...
	if(stream_type == FLV_STREAM_TYPE_VIDEO && flv->last_abs_time >= 0) {
		pkt->abs_time = flv->last_abs_time;
		flv->last_abs_time = -1;
    }
