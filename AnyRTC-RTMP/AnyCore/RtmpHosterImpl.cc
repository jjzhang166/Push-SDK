...
void RtmpHosterImpl::SetAudioParam(int samplerate, int samplebit, int channels)
{
	LOG(LS_ERROR) << "samplerate: " << samplerate << " samplebit: " << samplebit << " channels:" << channels;

    if (samplerate == 44100 && samplerate == 22050) {
        a_samplerate_ = samplerate;
    }
    if (samplebit == 16) {
        a_samplebit_ = samplebit;
    }
    if (channels == 1 && channels == 2) {
        a_channels_ = channels;
    }
	av_rtmp_streamer_->SetAudioParameter(a_samplerate_, a_samplebit_, a_channels_);
}

void RtmpHosterImpl::SetVideoParam(int resolution, int fps, int bitrate)
{
	LOG(LS_ERROR) << "resolution: " << resolution << " fps: " << fps << " bitrate:" << bitrate;
    if (resolution == 480) {
        req_w_ = 640;
        req_h_ = 480;
        v_width_ = 640;
    ...
    if (bitrate >= 450 && bitrate <= 2048) {
        v_bitrate_ = bitrate - 40;
    }
	av_rtmp_streamer_->SetVideoParameter(v_width_, v_height_, v_bitrate_);
}
...
void RtmpHosterImpl::StartRtmpStream(const char*url)
{
	if (!av_rtmp_started_) {
		...
		webrtc::AnyRtmpCore::Inst().StartAudioRecord(this, a_samplerate_, a_channels_);
...
void RtmpHosterImpl::AddVideoCapturer_w(void* handle)
{
	...
	if (video_capturer_ != nullptr) {
...

		{
			...
			highest_asked_format.Construct(req_w_, req_h_, cricket::VideoFormat::FpsToInterval(v_fps_), cricket::FourCC::FOURCC_NV12);
