...
static const size_t kEventMaxWaitTimeMs = 25;
		...
		const size_t nChannels, const uint32_t samplesPerSec, const uint32_t totalDelayMS)
{
	...
	if(encoder_)
	{
		...
		if(outlen > 0)
		{
			callback_.OnEncodeDataCallback(true, encoded, outlen, curtime, NULL);
		}
	}
...
, encoder_(NULL)
{
    ...
    if (!running_) {
        running_ = true;
        rtc::Thread::Start();
        deliver_buffer_event_->StartTimer(false, kEventMaxWaitTimeMs);
    }
}
...
void V_H264Encoder::CreateVideoEncoder()
{
	...
	LOG(LS_ERROR) << "extern_encoder:"<< (void*)extern_encoder;
...
{
	while(running_ && kEventError != deliver_buffer_event_->Wait(kEventMaxWaitTimeMs))
	{
        ...
        if (frame_to_render) {
            ...
            if(encoder_ == NULL)
            {
                LOG(LS_ERROR) << "h264_.width: "<< h264_.width <<" h264_.height: "<<h264_.height;
                CreateVideoEncoder();
            }
            ...
            if(encoder_)
            {
                LOG(LS_INFO) << "before encoder_->Encode";
...
{
    uint32_t delay_Ms = 0;
    int32_t isDropping = 0;
	callback_.OnEncodeDataCallback(false, encoded_image._buffer, encoded_image._length, rtc::Time(), &delay_Ms);
    if (delay_Ms >= 3000) {
        isDropping = 1;
    } else {
        isDropping = 0;
