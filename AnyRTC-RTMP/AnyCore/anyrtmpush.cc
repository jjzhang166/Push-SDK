...
uint32_t AnyRtmpPush::GetDelayMs() {
	uint32_t delayMs = 0;
    rtc::CritScope l(&cs_list_enc_);
...
void print_kbps(int &nbytes_in_ts, int &first_ts, int &last_ts, EncData* dataPtr, const char *type) {
    if (last_ts - first_ts <= 2000) {
        nbytes_in_ts += dataPtr->_dataLen;
    } else {
        LOG(LS_ERROR) << type <<" kbps: "<< nbytes_in_ts * 8.0 / 2.0 / 1024.0;
        nbytes_in_ts = dataPtr->_dataLen;
        first_ts = 0;
        last_ts = 0;
    }
    if (first_ts == last_ts) {
        first_ts = dataPtr->_dts - 1;
    }
    last_ts = dataPtr->_dts;
}
...
void AnyRtmpPush::DoSendData()
{
	...
	if (dataPtr != NULL) {
		if (dataPtr->_type == VIDEO_DATA) {
            ...
            static int nbytes_in_ts = 0;
            static int first_ts = 0;
            static int last_ts = 0;
            print_kbps(nbytes_in_ts, first_ts, last_ts, dataPtr, "video");

			LOG(LS_ERROR) << "doSendData VIDEO len: "<< dataPtr->_dataLen <<" dts: " << dataPtr->_dts << " type: " << type;
            ...
            print_kbps(nbytes_in_ts, first_ts, last_ts, dataPtr, "audio");

			int ret = 0;
			LOG(LS_ERROR) << "doSendData AUDIO len: "<< dataPtr->_dataLen <<" dts: " << dataPtr->_dts;
