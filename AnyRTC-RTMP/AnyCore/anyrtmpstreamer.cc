...
void AnyRtmpStreamerImpl::SetAudioParameter(int a_sample_hz, int bitpersample, int a_channels)
{
    a_sample_hz_ = a_sample_hz;
    // (void)bitpersample;
    a_channels_ = a_channels;
}
...
void AnyRtmpStreamerImpl::OnEncodeDataCallback(bool audio, uint8_t *p, uint32_t length, uint32_t ts, uint32_t *delayMs)
{
	if(audio)
	{
