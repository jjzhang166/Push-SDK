...
@property(nonatomic,strong)NSTimer* detechAuthorTimer;
@property(nonatomic,assign)BOOL isAppOnScreen;
...
- (id)initWithSessionPreset:(NSString *)sessionPreset cameraPosition:(AVCaptureDevicePosition)cameraPosition;
{
    ...
    self.isAppOnScreen = YES; //default value
    ...
    if (authStatus==AVAuthorizationStatusNotDetermined) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:PA_CAMERA_AUTHORIZE_FAIL object:nil];
    }
    ////////////
...
//        conn.videoMaxFrameDuration = CMTimeMake(1,60);
    ...
    //register AVCaptureSessionDidStopRunningNotification
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(doAVCaptureSessionDidStopRunningNotification:) name:AVCaptureSessionDidStopRunningNotification object:nil];
    
    
	return self;
}

-(void)doAVCaptureSessionDidStopRunningNotification:(id)sender{
...
- (void)rotateCamera:(AVCaptureDevicePosition)devicePosition
{
...
- (void)updateTargetsForVideoCameraUsingCacheTextureAtWidth:(int)bufferWidth height:(int)bufferHeight time:(CMTime)currentTime;
{
    ...
    if([self.delegate respondsToSelector:@selector(didOutputVideoSampleBuffer:)])
    {
        ...
        [self.delegate didOutputVideoSampleBuffer:[outputFramebuffer pixelBuffer]];
...
//        if (captureAsYUV && [GPUImageContext deviceSupportsRedTextures])
        if (CVPixelBufferGetPlaneCount(cameraFrame) > 0) // Check for YUV planar inputs to do RGB conversion
        {
            ...
            {
                bufferWidth = imageBufferWidth = self.outputHeight;
                bufferHeight = imageBufferHeight = self.outputWidth;
            }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    else
    {
        ...
        runAsynchronouslyOnVideoProcessingQueue(^{
            ...
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(willOutputVideoSampleBuffer:)])
            {
                [weakSelf.delegate willOutputVideoSampleBuffer:sampleBuffer];
            }
...
- (void)setAppOnScreen:(BOOL)onScreen{
    NSLog(@"setAppOnScreen :%d",onScreen);
    self.isAppOnScreen = onScreen;
